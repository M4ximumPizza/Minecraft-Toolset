...
void read_nbt_impl(char ** const pfirst, char *last, MC-Toolkit::nbt_type type,
    std::string_view name, std::vector<MC-Toolkit::nbt_t> *v);

MC-Toolkit::nbt_type read_type(char ** const pfirst, char *last) {
    ...
    using nbt_type = MC-Toolkit::nbt_type;
...
template<typename T>
inline
std::enable_if_t<std::is_arithmetic_v<T>,MC-Toolkit::nbt_t::payload_t>
read_payload_impl(char ** const pfirst, char *last) {
    assert(pfirst != nullptr && *pfirst != nullptr);
    assert(last != nullptr);
    assert(*pfirst <= last);
    using payload_t = MC-Toolkit::nbt_t::payload_t;
...
std::enable_if_t<std::is_member_pointer_v<decltype(&T::data)> &&
    std::is_member_pointer_v<decltype(&T::size)>, MC-Toolkit::nbt_t::payload_t>
read_payload_impl(char ** const pfirst, char *last) {
    assert(pfirst != nullptr && *pfirst != nullptr);
    assert(last != nullptr);
    assert(*pfirst <= last);
...
inline
void
read_simple_payload(char ** const pfirst, char *last, MC-Toolkit::nbt_type type,
    std::string_view name, std::vector<MC-Toolkit::nbt_t> *v) {
    using nbt_type = MC-Toolkit::nbt_type;
    MC-Toolkit::nbt_t::payload_t payload;
    ...
    case nbt_type::BYTE_ARRAY:
        payload = read_payload_impl<MC-Toolkit::nbt_byte_array_t>(pfirst, last);
        break;
    case nbt_type::STRING:
        payload = read_payload_impl<MC-Toolkit::nbt_string_t>(pfirst, last);
        break;
    case nbt_type::INT_ARRAY:
        payload = read_payload_impl<MC-Toolkit::nbt_int_array_t>(pfirst, last);
        break;
    case nbt_type::LONG_ARRAY:
        payload = read_payload_impl<MC-Toolkit::nbt_long_array_t>(pfirst, last);
        break;
    default:
        return;
    }
...
inline
void read_list_payload(char ** const pfirst, char *last, MC-Toolkit::nbt_type type,
    std::string_view name, std::vector<MC-Toolkit::nbt_t> *v) {
    assert(pfirst != nullptr && *pfirst != nullptr);
    assert(last != nullptr);
    assert(*pfirst <= last);
    ...
    v->emplace_back(name, MC-Toolkit::nbt_list_t{list_size, list_type});
    ...
    *pfirst = p;
    v->emplace_back(std::string_view{}, payload_t{MC-Toolkit::nbt_list_end_t{}});
}

inline
void read_compound_payload_impl(char ** const pfirst, char *last,
    std::vector<MC-Toolkit::nbt_t> *v) {
    assert(pfirst != nullptr && *pfirst != nullptr);
    assert(last != nullptr);
    assert(*pfirst <= last);
    assert(v != nullptr);
...
inline
void read_compound_payload(char ** const pfirst, char *last, MC-Toolkit::nbt_type type,
    std::string_view name, std::vector<MC-Toolkit::nbt_t> *v) {
    assert(pfirst != nullptr && *pfirst != nullptr);
    assert(last != nullptr);
    assert(*pfirst <= last);
    ...
    v->emplace_back(name, MC-Toolkit::nbt_compound_t{});
    ...
    v->emplace_back(std::string_view{}, MC-Toolkit::nbt_end_t{});
}
...
bool read_nbt(char *first, size_t length, std::vector<MC-Toolkit::nbt_t> *nbt_data) {
...
int main(int argc, char *argv[]) {
    ...
    std::string buffer = MC-Toolkit::slurp_string(std::cin);

    std::vector<MC-Toolkit::nbt_t> result;
